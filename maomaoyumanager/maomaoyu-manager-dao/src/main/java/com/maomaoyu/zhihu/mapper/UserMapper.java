...
public interface UserMapper {
    ...
    @Insert({"insert into ",TABLE_NAME," (",INSERT_FIELD,") values(#{name},#{password},#{salt},#{headUrl},#{email},#{status})"})
    ...
    @Update({"update ",TABLE_NAME," set name=#{name},password=#{password},salt=#{salt},head_url=#{headUrl},email=#{email} where id=#{id} and status=0"})
